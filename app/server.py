...
_ALLOWED_EXTENSIONS = {".csv", ".ofx", ".qfx", ".xlsx", ".xls", ".pdf", ".html", ".mhtml"}
...
def _review_handler(space: str, back_url: str):
    ...
    if request.method == "POST":
        ...
        if action == "skip":
            ...
            patrimony_label = request.form.get("patrimony_label", "").strip() or None
            ...
            if txn_row:
                conn.execute(
                    "UPDATE transactions SET category = ?, subcategory = ?, notes = ?, patrimony_label = ?, verified = 1 WHERE id = ? AND space = ?",
                    (category, subcategory, notes, patrimony_label, txn_id, space)
                )
                conn.commit()
                mappings = _load_mappings(MAPPINGS_PATH, space)
